...
{
    private static readonly Uri WebhookUrl = new(
    ...
    public DiscordWebhookDispatcherTests()
    {
        ...
        this._dispatcher = new DiscordWebhookDispatcher(
            httpClientFactory: this._httpClientFactory,
            options: Options.Create(options),
            logger: this.GetTypedLogger<DiscordWebhookDispatcher>()
        );
    }
    ...
    public async Task SendAsyncDoesNotCallHttpClientWhenWebhookUrlIsNullAsync()
    {
        ...
        IDiscordDispatcher dispatcher = new DiscordWebhookDispatcher(
            httpClientFactory: this._httpClientFactory,
            options: Options.Create(optionsWithNoWebhook),
            logger: this.GetTypedLogger<DiscordWebhookDispatcher>()
        );
    ...
    public async Task SendAsyncCallsDiscordHttpClientWhenWebhookUrlIsSetAsync()
    {
        ...
        await this._dispatcher.SendAsync(
            message: message,

...
public interface INotificationStateTracker
{
    ...
    Task UpdatePullRequestStateAsync(string repository, int pullRequestNumber, string status, WorkPriority priority, bool isOnHold, CancellationToken cancellationToken);
    ...
    Task UpdateIssueStateAsync(string repository, int issueNumber, string status, WorkPriority priority, bool isOnHold, bool hasLinkedPr, CancellationToken cancellationToken);
}
